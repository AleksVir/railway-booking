...
function PassengerCard({
   ...
   const thisPassenger = passengers.filter((pas) => pas.id === id);
   const coaches = useSelector(selectSelectedCoaches);

   const safeUnchosenSeats = unchosenSeats || [];
   const safeUnchosenSeatsDep = unchosenSeatsDep || [];
   const safeUnchosenSeatsArr = unchosenSeatsArr || [];

   const [expanded, setExpanded] = useState(true);
   const [showError, setShowError] = useState(false);
   const [departureOnly, setDepartureOnly] = useState(
      !safeUnchosenSeatsArr.length
   );
   ...
   if (thisPassenger.length > 0) {
      ...
      if (thisPassenger[0].dateOfBirth) {
         ...
         initialValues.dateOfBirth = dayjs(`${data[2]}-${data[1]}-${data[0]}`);
      }
   }
   ...
   const seatsFilter = useCallback(
      (coef) => {
         setUnchosenSeatsDepSourse(
            safeUnchosenSeatsDep.filter((el) => el.priceCoefficient === coef)
         );

         if (safeUnchosenSeatsArr.length > 0 && !departureOnly) {
            setUnchosenSeatsArrSourse(
               safeUnchosenSeatsArr.filter((el) => el.priceCoefficient === coef)
            );
         }
      },
      [departureOnly, safeUnchosenSeatsArr, safeUnchosenSeatsDep]
   );
   ...
   const onFinish = (values) => {
      if (!values[fieldNames.dateOfBirth]) {
         setShowError(true);
         return;
      }
      ...
      if (!thisPassenger[0] && !disabledForwardBtn) {
         ...
         dispatch(addNewPassenger(preparedValues));

         if (
            safeUnchosenSeatsDep.length - (values.seatDep ? 1 : 0) > 0 ||
            safeUnchosenSeatsArr.length - (values.seatArr ? 1 : 0) > 0
         ) {
         ...
         dispatch(editPassengerData(preparedValues));
   ...
   const fieldChangeHandler = (value) => {
      if (value[fieldNames.passengerType]) {
         ...
         form.setFieldValue(fieldNames.docType, doc);
         setDocumentType(doc);
   ...
   const passType = (
      <Form.Item name={fieldNames.passengerType} rules={rules.passengerType}>
         <Select
            className="passengerCard-select"
            classNames={{
               popup: {
                  root: "passengerCard-select",
               },
            }}
         >
            {unchosenSeatsAdults > 0 && (
               <Select.Option value={passengerTypes.adults}>
                  {pasTypesRus[passengerTypes.adults]}
               </Select.Option>
            )}

            {unchosenSeatsChildren > 0 && (
   ...
   const seatDep = (
      <Form.Item
         ...
         rules={rules.seatDep}
      >
         <Select className="passengerCard-seat-select" allowClear={false}>
...
PassengerCard.propTypes = {
   ...
   unchosenSeats: PropTypes.arrayOf(PropTypes.shape(shapedObj)),
   unchosenSeatsDep: PropTypes.arrayOf(PropTypes.shape(shapedObj)),
